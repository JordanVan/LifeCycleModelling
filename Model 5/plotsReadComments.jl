...
function plotLifetimeConsumption(initialWealth::Float64,annuitizationLevel::Float64;noisy::Bool=true)
    ...
    totalUtility = 0.0
    probOfLiving = 1.0
    annCost = annuitizationLevel*initialWealth/100.0
    currentWealth = initialWealth-annCost
    consumed = zeros(PeriodsToAdd+1)
    wealthVec = zeros(PeriodsToAdd+1)
    savVec = zeros(PeriodsToAdd+1)
    totalConsumed = zeros(PeriodsToAdd+1)
    anuVec = [fairCouponRate(annCost) for i in 0:PeriodsToAdd]
    ...
    for i in 0:PeriodsToAdd
        ...
        availableFunds = currentWealth + fairCouponRate(annCost)
        consumed[i+1] = consumptionFunction(currentWealth+fairCouponRate(annCost))
        #consumed[i+1] = fairCouponRate(annuitizationLevel)
        if (noisy == true)
            print("I am $currentAge and have $availableFunds. ")
        end
        savVec[i+1] = currentWealth + fairCouponRate(annCost) - consumed[i+1]
        currentWealth = currentWealth + fairCouponRate(annCost) - consumed[i+1]
        ...
        probOfLiving *= Φ(retirementAge+i+1,0)
